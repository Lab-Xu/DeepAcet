...
with tf.name_scope('input'):
    ...
    x=tf.placeholder(tf.float32,[None,2199])
...
with tf.name_scope('layer'):
    with tf.name_scope('w1'):
        W1=tf.Variable(tf.truncated_normal([2199,20],stddev=0.1))
...
def Save_evaluation(data,name):
    ...
    writer = csv.writer(csvfile)
    writer.writerow(['Accuracy', 'Specitivity','Sensitivity','AUC','MCC'])
...
def DNN(train_file,test_file,Isrestore):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(60):
            ...
            if Isrestore == 0:
                ...
                for batch in range(n_batch):
                    ...
                    summary= sess.run(merged, feed_dict={x: batch_xs, y: batch_ys, keep_prob: 0.9})
            else:
            ...
            if Isrestore != 0:
                target.append([Accuracy, Specitivity, Sensitivity, AUC,MCC])
                print('Accuracy:' + str(Accuracy) + '\nSpecitivity:' + str(Specitivity) + '\nSensitivity:' + str(
                    Sensitivity) + '\nAUC:' + str(AUC)+'\nMCC:' + str(MMC))
                Save_evaluation(target, 'reload_target' + str(Isrestore) + '.csv')
                break
            if Isrestore == 0:
                target.append([Accuracy,Specitivity,Sensitivity,AUC,MCC])
            ...
            if Isrestore == 0:
                if Accuracy>MaxACC:
                    MaxACC=Accuracy
                    resultACC = [epoch+1,len(test_pre),Accuracy,Specitivity,Sensitivity,AUC,MCC]
                if AUC>MaxAUC:
                    MaxAUC=AUC
                    resultAUC = [epoch + 1,len(test_pre), Accuracy, Specitivity, Sensitivity, AUC,MCC]
        ...
        # print(test_pre)
        if Isrestore==0:
            Save_evaluation(target, 'target' + str(epoch + 1) + '.csv')
...
def K_cross(train_file,test_file,k,isrestore):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(60):
            ...
            if isrestore==0:
                ...
                test_prob_combine = []
                for i in range(k):
                    test_feature_cross = data_feature[n * i:n * (i + 1)]
                    test_label_cross = data_label[n * i:n * (i + 1)]
                    train_feature_cross = data_feature[0:n * i] + data_feature[n * (i + 1):k * n]
                    train_label_cross = data_label[0:n * i] + data_label[n * (i + 1):k * n]
                    for batch in range(n_batch):
                        batch_xs = train_feature_cross[batch * batch_size:(batch + 1) * batch_size]
                        batch_ys = train_label_cross[batch * batch_size:(batch + 1) * batch_size]
                        sess.run(train_step, feed_dict={x: batch_xs, y: batch_ys, keep_prob: 0.9})  # 训练0.
                    test_prob= sess.run(prediction, feed_dict={x: test_feature_cross, y: test_label_cross, keep_prob: 1.0})
                    test_pre = JudgePositive(test_prob, 0.5)
                    test_pre_combine.extend(test_pre)
                    test_prob_combine.extend(test_prob)
                saver.save(sess, './Model/DNN_ten_cross_model_' + str(epoch + 1) + '.ckpt')
                ...
                if Accuracy > cross_MaxACC:
                    ...
                    cross_resultACC = [epoch + 1, len(test_prob_combine), Accuracy, Specificity, Sensitivity, AUC,MCC]
                if AUC > cross_MaxAUC:
                    cross_MaxAUC = AUC
                    cross_resultAUC = [epoch + 1, len(test_prob_combine), Accuracy, Specificity, Sensitivity, AUC,MCC]
            else:
                saver.restore(sess, './Model/DNN_ten_cross_model_' + str(isrestore) + '.ckpt')

            #independent dataset
            test_prob_ind = sess.run(prediction, feed_dict={x: test_feature, y: test_label, keep_prob: 1.0})
            test_pre_ind = JudgePositive(test_prob_ind, 0.5)

            for c in range(len(test_pre_ind)):
                if test_pre_ind[c] == test_label[c][0]:
                    if test_pre_ind[c] == 1:
                        TP += 1
                    else:
                        TN += 1
            ...
            Sensitivity = TP / (TP + FN)
            AUC, TPR, FPR = ROC(test_prob_ind, test_label, epoch + 1, 0)
            MCC = (TP * TN - FP * FN) / math.sqrt((TP + FP) * (TP + FN) * (TN + FP) * (TN + FN))
            ...
            if isrestore==0:
                ...
                if Accuracy > MaxACC:
                    MaxACC = Accuracy
                    resultACC = [epoch + 1, len(test_pre_ind), Accuracy, Specificity, Sensitivity, AUC,MCC]

                if AUC > MaxAUC:
                    MaxAUC = AUC
                    resultAUC = [epoch + 1, len(test_pre_ind), Accuracy, Specificity, Sensitivity, AUC,MCC]
            else:
                Save_evaluation(evaluation,'restore_independent_evaluation_'+str(isrestore)+'.csv')
                Save_csv([TPR, FPR], 'restore_DNN_epoch_independent_' + str(isrestore)+'.csv')
        ...
        if isrestore==0:
            print("ten_cross_MaxAUC: Iter: " + str(cross_resultAUC[0]) + "\nnumber:" + str(
                ...
                cross_resultAUC[4]) + "\nAUC:"+ str(cross_resultAUC[5])+"\nMCC:"+ str(cross_resultAUC[6]))
...
if __name__=='__main__':
    ...
    if is_ten_cross==0:
        train='Homo_train_random_fscore.csv'
        test='Homo_test_random_fscore.csv'
        DNN(train,test,0)

    else:
        train = 'Homo_train_random_fscore.csv'
        test = 'Homo_test_random_fscore.csv'
        K_cross(train,test,10,60)
