...
with tf.name_scope('input'):
    ...
    x=tf.placeholder(tf.float32,[None,2205])
...
with tf.name_scope('layer'):
    with tf.name_scope('w1'):
        W1=tf.Variable(tf.truncated_normal([2205,20],stddev=0.1))
...
if __name__=='__main__':
    ...
    if is_ten_cross==0:
        train='Homo_15_trainKSPACE5_random.csv'
        test='Homo_15_testKSPACE5_random.csv'
        DNN(train,test,0)
